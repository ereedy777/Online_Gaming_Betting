...
#3 Year over Yaer (YOY) - Average GGR 
SELECT Fiscal_Year,
AVG(GGR) AS Avg_GGR
FROM online_sports_betting
WHERE Licensee = 'MPI Master Wagering License CT, LLC'
...
#Which months within Q4 have people spent most money
SELECT date,
FORMAT(Wag,0) AS Wag
FROM online_sports_betting
WHERE Licensee = 'MPI Master Wagering License CT, LLC'
...
#7 Hold percentage (Higher the higher GGR relative to how much wagered)
SELECT Fiscal_Year, 
date,
...
#9 Retrun to Player(RTP) 96% - considred generally good RTP
SELECT  Licensee,
date,
Win_rate,
GGR
...
#10 NGR to Promotional Expenses Ratio
SELECT Fiscal_Year,
